...
/**
 * CryptomusClient — USDT / crypto via Cryptomus hosted checkout.
 ...
 * URL; the payer pays crypto there, and Cryptomus POSTs a webhook on status
...
{
    private const INVOICE_URL = 'https://api.cryptomus.com/v1/payment';
    private const INFO_URL = 'https://api.cryptomus.com/v1/payment/info';
    ...
    public function initiate(array $data): array
    {
        if (!$this->isConfigured()) {
            return ['success' => false, 'message' => 'Cryptomus is not configured.'];
        }
        ...
        $msg = $result['message'] ?? ($result['errors'] ? json_encode($result['errors']) : 'Unknown Cryptomus error');
    ...
    {
        if ($this->apiKey === '') {
            error_log('[CryptomusClient] WARNING: apiKey empty — webhook signature NOT verified');
    ...
    /** Cryptomus terminal "paid" statuses. */
